...
class AgendaController extends Controller
{
    ...
    {
        $query = Agenda::query()->where('status', 'published')->where('is_active', true)->orderBy('start_date', 'asc');
        ...
        // Sidebar Data
        $categories = Agenda::where('status', 'published')->where('is_active', true)
            ->whereNotNull('category')->where('category', '!=', '')
            ->selectRaw('category, count(*) as total')
            ->groupBy('category')->get();

        $upcomingAgendas = Agenda::where('status', 'published')->where('is_active', true)
            ->where('start_date', '>=', now()->toDateString())
            ->orderBy('start_date', 'asc')->take(4)->get();
        
        $archives = Agenda::where('status', 'published')->where('is_active', true)
