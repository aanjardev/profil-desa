...
@section('page_title', 'Tambah Data Wisata')
@section('page_subtitle', 'Masukkan informasi detail mengenai potensi wisata desa.')

@section('content')
<form action="{{ route('admin.tourisms.store') }}" method="POST" enctype="multipart/form-data" 
      x-data="{ images: [], tickets: [], spots: [] }">
    @csrf
                ...
                <div class="space-y-5">
                    ...
                    <div>
                        ...
                        <textarea name="description" id="description" rows="8" required
                                  ...
                                  placeholder="Ceritakan sejarah, keunikan, dan hal menarik dari wisata ini...">{{ old('description') }}</textarea>
                        @error('description')<p class="mt-1 text-sm text-red-500">{{ $message }}</p>@enderror
                    </div>

                    <div>
                        <label for="opening_hours" class="block text-sm font-bold text-gray-700 mb-1">Jam Operasional (Opsional)</label>
                        <input type="text" name="opening_hours" id="opening_hours" value="{{ old('opening_hours') }}" maxlength="255"
                               class="w-full px-4 py-2.5 bg-gray-50 border border-gray-200 rounded-lg focus:bg-white focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-colors text-sm"
                        ...
                        <div class="flex items-start gap-3 p-3 bg-gray-50 rounded-lg border border-gray-200">
                            <div class="flex-1 space-y-3">
                                <div>
                                    ...
                                    <input type="text" :name="'tickets['+index+'][name]'" required class="w-full px-3 py-2 text-sm bg-white border border-gray-200 rounded-md" placeholder="Contoh: Reguler / VIP / Camping">
                                </div>
                                <div>
                                    <label class="block text-xs font-bold text-gray-600 mb-1">Harga (Opsional)</label>
                                    <input type="text" :name="'tickets['+index+'][price]'" class="w-full px-3 py-2 text-sm bg-white border border-gray-200 rounded-md" placeholder="Contoh: Rp 15.000">
                                </div>
                                <div>
                                    <label class="block text-xs font-bold text-gray-600 mb-1">Keterangan (Opsional)</label>
                                    <input type="text" :name="'tickets['+index+'][description]'" class="w-full px-3 py-2 text-sm bg-white border border-gray-200 rounded-md" placeholder="Contoh: Termasuk makan siang">
                                </div>
                            </div>
                        ...
                        <div class="flex items-start gap-3 p-3 bg-gray-50 rounded-lg border border-gray-200">
                            <div class="flex-1 space-y-3">
                                <div>
                                    ...
                                    <input type="text" :name="'spots['+index+'][name]'" required class="w-full px-3 py-2 text-sm bg-white border border-gray-200 rounded-md" placeholder="Contoh: Jeep Offroad / Flying Fox">
                                </div>
                                <div>
                                    <label class="block text-xs font-bold text-gray-600 mb-1">Harga Tambahan (Opsional)</label>
                                    <input type="text" :name="'spots['+index+'][price]'" class="w-full px-3 py-2 text-sm bg-white border border-gray-200 rounded-md" placeholder="Contoh: Rp 150.000 / Gratis">
                                </div>
                                <div>
                                    <label class="block text-xs font-bold text-gray-600 mb-1">Keterangan Singkat (Opsional)</label>
                                    <input type="text" :name="'spots['+index+'][description]'" class="w-full px-3 py-2 text-sm bg-white border border-gray-200 rounded-md" placeholder="Contoh: Termasuk supir dan bensin">
                                </div>
                                <div>
                                    <label class="block text-xs font-bold text-gray-600 mb-1">Link Pesan Wahana Online (Opsional)</label>
                                    <input type="url" :name="'spots['+index+'][order_link]'" class="w-full px-3 py-2 text-sm bg-white border border-gray-200 rounded-md" placeholder="https://...">
                                </div>
                                <div>
                                    <label class="block text-xs font-bold text-gray-600 mb-1">Gambar Wahana (Opsional)</label>
                                    <div class="flex items-center gap-3">
                                        <!-- Preview Box -->
                                        <div class="w-16 h-16 rounded-md bg-gray-100 border border-gray-300 flex items-center justify-center overflow-hidden shrink-0 relative" :class="{'bg-white': spot.preview}">
                                        ...
                                        <div class="flex-1">
                                            <label class="cursor-pointer inline-flex items-center px-3 py-1.5 border border-gray-300 shadow-sm text-xs font-semibold rounded text-gray-700 bg-white hover:bg-gray-50 transition-colors">
                                                <span x-text="spot.preview ? 'Ganti Foto' : 'Pilih Foto'"></span>
                ...
                <div class="space-y-5">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                        <div>
                            ...
                            <input type="text" name="maps_link" id="maps_link" value="{{ old('maps_link') }}"
                                   class="w-full px-4 py-2.5 bg-gray-50 border border-gray-200 rounded-lg focus:bg-white focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-colors text-sm"
                        ...
                        <div>
                            ...
                            <input type="url" name="digital_map_link" id="digital_map_link" value="{{ old('digital_map_link') }}" maxlength="255"
                                   class="w-full px-4 py-2.5 bg-gray-50 border border-gray-200 rounded-lg focus:bg-white focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-colors text-sm"
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                        <div>
                            ...
                            <input type="url" name="instagram_link" id="instagram_link" value="{{ old('instagram_link') }}" maxlength="255"
                                   class="w-full px-4 py-2.5 bg-gray-50 border border-gray-200 rounded-lg focus:bg-white focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-colors text-sm"
                        ...
                        <div>
                            ...
                            <input type="url" name="youtube_link" id="youtube_link" value="{{ old('youtube_link') }}" maxlength="255"
                                   class="w-full px-4 py-2.5 bg-gray-50 border border-gray-200 rounded-lg focus:bg-white focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-colors text-sm"
                    ...
                    <div>
                        ...
                        <input type="url" name="order_link" id="order_link" value="{{ old('order_link') }}" maxlength="255"
                               class="w-full px-4 py-2.5 bg-gray-50 border border-gray-200 rounded-lg focus:bg-white focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-colors text-sm"
                    ...
                    <div>
                        ...
                        <textarea name="facilities" id="facilities" rows="3"
                                  class="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-lg focus:bg-white focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-colors text-sm"
                                  placeholder="Contoh: Area parkir luas, Toilet bersih, Warung makan, Mushola">{{ old('facilities') }}</textarea>
        ...
        <div class="space-y-6">
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6" 
                ...
                <div class="flex flex-col gap-1 mb-3 border-b border-gray-100 pb-3">
                    ...
                    <p class="text-[11px] text-gray-500 leading-tight">Wajib. Gambar akan di-crop otomatis ke rasio <span class="font-bold">16:9</span>. Maksimal <span class="font-bold">2MB</span>.</p>
                </div>
    ...
    <div class="mt-6 bg-white rounded-xl shadow-sm border border-gray-100 p-6">
        ...
        <div class="space-y-4">
            ...
            <div x-show="images.length === 0" class="text-center py-8 bg-gray-50/50 rounded-lg border border-dashed border-gray-200">
                <p class="text-sm text-gray-500">Belum ada foto galeri. Klik tombol "Tambah Gambar Galeri".</p>
            </div>
            ...
            <template x-for="(img, index) in images" :key="img.id">
                <div class="relative flex flex-col md:flex-row items-start gap-4 p-4 bg-gray-50 border border-gray-200 rounded-xl group transition-all hover:border-blue-300 hover:shadow-sm">
                    ...
                    <div class="flex-1 w-full flex flex-col justify-between h-full space-y-3">
                        <div>
                            <label class="block text-xs font-bold text-gray-600 mb-1">Keterangan Foto (Opsional)</label>
